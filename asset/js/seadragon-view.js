$(document).ready(function () {
  $('.openseadragon').each(function () {
    ...
    // Don't load the viewer until it's visible on the page or the initial Zoom will derp
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          // Element is visible
          loadViewer(currentViewer, currentViewerID, iiifEndpoint, authtoken, thumbnail, expiration);
          observer.unobserve(currentViewer[0]);
        }
      });
    });
